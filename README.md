...
> 版本: V2.1 — 多策略加权投票 + 动态品种池 + CZSC缠论 + Telegram通知
...
- **动态交易品种池 V2.1**: 实盘+模拟盘双重验证，每日自动筛选，上限 30 个品种。实盘成交额>8000万U ∪ 涨幅前10 ∪ 跌幅前10，与模拟盘取交集（模拟盘涨幅/跌幅取前30），按成交额排序截取前 30
...
### V2.1（2026-05-11）— 系统优化
- 🚀 品种池上限 30 个（MAX_SYMBOLS），60秒周期内可完成全量分析
- 🎯 信号置信度阈值统一 0.7（代码+风控一致）
- 🔒 线程安全：账户/信号/K线/品种共享数据加锁保护
- 📩 通知模块异步化（队列+独立线程，不再阻塞主循环）
- 🛡️ 熔断冷却递增：60s→120s→240s→...→最大15分钟
- 📊 K线保留 6h→24h（缠论策略需要更长历史）
- 🧪 回测引擎适配 JSONL 格式（优先本地数据）
- 🔧 回测杠杆统一从风控配置读取
- 🔐 .gitignore 完善：data/ 全目录 + config/symbols.json 不提交
